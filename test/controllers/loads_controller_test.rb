...
class LoadsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create load" do
    assert_difference("Load.count") do
      post loads_url, params: { load: { delivery_date: @load.delivery_date, destination: @load.destination, origin: @load.origin, pickup_date: @load.pickup_date, status: @load.status } }
    end
  ...
  test "should update load" do
    patch load_url(@load), params: { load: { delivery_date: @load.delivery_date, destination: @load.destination, origin: @load.origin, pickup_date: @load.pickup_date, status: @load.status } }
